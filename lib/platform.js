...
export default class {
    ...
    try {
      ...
      if (this.cloudClient || this.config.userkey) {
        ...
        Object.values(this.deviceConf)
          ...
          .forEach(async (device) => this.initialiseDevice(device));
      } else {
      ...
      setTimeout(() => this.log('%s. %s', platformLang.complete, platformLang.zWelcome[randIndex]), 2000);
    } catch (err) {
      // Catch any errors during setup
      const eText = parseError(err, [platformLang.disabled, platformLang.noCredentials]);
  ...
  async initialiseDevice(device) {
    try {
      ...
      if (platformConsts.models.switchSingle.includes(device.model)) {
        ...
        accessory = this.devicesInHB.get(hbUUID) || this.addAccessory(device);
        accessory.context = { ...accessory.context, ...context };
        accessory.control = new deviceTypes.deviceRoller(this, accessory);
        /** ******** */
        ...
        // Update any changes to the accessory to the platform
        this.api.updatePlatformAccessories(plugin.name, plugin.alias, [accessoryLight]);
        ...
        subdevices.forEach((subdevice) => {
          try {
            ...
            const eText = parseError(err);
            this.log.warn('[%s] %s %s.', subdevice.subDeviceName, platformLang.devNotAdd, eText);
          }
        });
        /** ****** */
      } else {
      ...
      this.log('[%s] %s [%s].', accessory.displayName, platformLang.devInit, device.uuid);

      // Extra debug logging when set, show the device JSON info
      if (accessory.context.enableDebugLogging) {
        this.log('[%s] %s: %s.', accessory.displayName, platformLang.jsonInfo, JSON.stringify(device));
      }
    ...
    try {
      ...
      if (!hidden) {
        accessory
          ...
          .setCharacteristic(this.api.hap.Characteristic.SerialNumber, this.api.hap.uuid.generate(device.hbDeviceID))
      ...
      const eText = parseError(err);
      this.log.warn('[%s] %s %s.', device.devName, platformLang.devNotAdd, eText);
      return false;
    }
  }
  ...
  addExternalAccessory(device, category) {
    try {
      ...
      // Catch any errors during add
      const eText = parseError(err);
      this.log.warn('[%s] %s %s.', device.name, platformLang.devNotAdd, eText);
      return false;
    }
  }
  ...
  removeAccessory(accessory) {
    try {
      ...
      const eText = parseError(err);
      this.log.warn('[%s] %s %s.', accessory.displayName, platformLang.devNotRemove, eText);
    }
  }
  ...
  async sendUpdate(accessory, toSend) {
    ...
    try {
      ...
      if (accessory.context.enableDebugLogging) {
        this.log('[%s] %s: %s.', accessory.displayName, platformLang.sendUpdate, JSON.stringify(data));
      }
      ...
      if (accessory.context.connection === 'local') {
        ...
        if (accessory.context.enableDebugLogging) {
          ...
          this.log('[%s] %s %s.', accessory.displayName, platformLang.revertToCloud, eText);
        }
